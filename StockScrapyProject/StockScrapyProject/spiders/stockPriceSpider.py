...
class stockPriceSpider(scrapy.Spider):
    ...
    def __init__(self,CSV_File_PATH, **kwargs):
        ...
        self.load_CSV()
        self.se_urls.append(f'https://www.tpex.org.tw/web/stock/aftertrading/daily_close_quotes/stk_quote_result.php?l=zh-tw&o=htm&d={self.tpex_SDate}&s=0,asc,0')
        self.se_urls.append(f'https://www.twse.com.tw/exchangeReport/MI_INDEX?response=html&date={self.twse_SDate}&type=ALLBUT0999')
    ...
    def check_se_parse(self,response):
        ...
        if(domain == 'www.twse.com.tw'):
            return_text_twse = str(response.xpath("//div[contains(text(),'很抱歉，沒有符合條件的資料!')]//text()").get())
            print(return_text_twse)
            self.is_TWSE_open=True if (return_text_twse == 'None') else False
        if(domain == 'www.tpex.org.tw'):
            return_text_tpex = str(response.xpath("//td[contains(text(),'共0筆')]//text()").get())
    ...
    def tpex_mining_Data_Parse(self,response):
        ...
        for data in response.xpath('body'):
            ...
            for co_id in local_Co_ids:
                ...
                print(co_id)
                tpex_get=''
                tpex_get=str(response.xpath(f'//td[text()="{co_id}"]//text()').get())
                tpex_co_name=str(data.xpath(f'//td[text()="{co_id}"]/following-sibling::td[1]//text()').get())
                print(tpex_get)
                if(tpex_get == 'None' and (not tpex_co_name.isnumeric())):
                    ...
                    tpex_price=str(data.xpath(f'//td[text()="{co_id}"]/following-sibling::td[2]//text()').get())
        ...
        if(not(self.is_TPEX_open and self.is_TWSE_open)):
            ...
            for data in response.xpath('body'):
                ...
                for co_id in local_Co_ids:
                    ...
                    print(co_id)
                    twse_get=''
                    twse_get=str(response.xpath(f'//td[text()="{co_id}"]//text()').get())
                    twse_co_name=str(data.xpath(f'//td[text()="{co_id}"]/following-sibling::td[1]//text()').get())
                    print(twse_get)
                    if(twse_get == 'None' and (not twse_co_name.isnumeric())):
                        ...
                        twse_price=str(data.xpath(f'//td[text()="{co_id}"]/following-sibling::td[6]//text()').get())
